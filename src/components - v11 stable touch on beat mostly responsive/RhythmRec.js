...
const RhythmRec = ({ start }) => {
  ...
  const checkHandler = (e) => {
    ...
    const mappedDivsList = divsList.map((item) => {
      const dimensions = item.getBoundingClientRect();
      const x = ((dimensions.x + dimensions.width / 2) / screenW) * 100;
      const y = ((dimensions.y + dimensions.height / 2) / screenH) * 100;
      return { x, y };
    });
